...
const cases = [_]Case{
    ...
    .{
        ...
        .decls = "pub const renderentry = {};",
        .expect = "declares unknown 'renderentry' — did you mean 'renderEntry'?",
    },
    .{
    ...
    .{
        // There are no editing hooks: a format that still spells one is
        // declaring a name the contract does not have.
        .name = "editing hook declared",
        .decls = "pub const insertKey = {};",
        .expect = "declares unknown 'insertKey'",
    },
    .{
    ...
    .{
        // The closed set covers renderers too, so a misspelled one is caught
        // the same way a misspelled hook is rather than silently never
        // dispatching.
        .name = "unknown decl, renderer typo",
        .decls = "pub const rendervalue = {};",
        .expect = "declares unknown 'rendervalue' — did you mean 'renderValue'?",
    },
    .{
        .name = "caps.edit = false with a renderer",
        .caps = ".{ .read = true, .edit = false, .serialize = true }",
        .decls = "pub const renderValue = {};",
        .expect = "declares caps.edit = false but supplies the editing renderer 'renderValue'",
    },
    .{
        // The whole-container ops are not declarations any more: every one
        // is a method the engine already has, driven by `section_noun`,
        // `section_header` and what the parser records.
        .name = "unknown decl, whole-container op",
        .decls = "pub const insertContainer = {};",
        .expect = "declares unknown 'insertContainer'",
    },
    .{
